...
class TimeSeries:
    ...
    def add_harmonics(self, freq: int = 5):
        if 't' not in self.independent:
            raise ValueError("You need to add time to time seies before added harmonics")
        if 'constant' not in self.independent:
            raise ValueError("You need to add Image Constant to time sereis before adding harmonics")
